...
interface Normal_MenuDessertDialogListener {
    fun onDessertMenuAdded(normalSelectedMenuInfo: NormalSelectedMenuInfo)
}

class Normal_MenuDessertDialog : DialogFragment() {

    private lateinit var binding: DialogNormalMenuDessertBinding
    private var dessertDialogListener: Normal_MenuDessertDialogListener? = null

    // NormalTakeOutAdapter랑 연결
    companion object {
        fun newInstance(item: NormalTakeOutVO): Normal_MenuDessertDialog {
            val args = Bundle().apply {
                putInt("normal_img", item.img)
                putString("normal_name", item.name)
                putString("normal_price", item.price)
            }
            val fragment = Normal_MenuDessertDialog()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (img != null) {
            binding.imgDessert.setImageResource(img)
        }
        binding.tvDessertName.text = name
        binding.tvDessertPrice.text = price
        ...
        binding.btnBasketN.setOnClickListener {
            ...
            dessertDialogListener?.onDessertMenuAdded(normalSelectedMenuInfo)
            dismiss()
        }
                    ...
                    if (price != null) {
                        ...
                        if (MenuPlusCountInt != null) {
                            ...
                            if (priceInt != null) {
                                ...
                                binding.tvDessertPrice.text = String.format("%,d원", plusPrice)
                            }
                        }
                    }
                }
    ...
    fun setDessertListener(listener: Normal_MenuDessertDialogListener) {
        this.dessertDialogListener = listener
    }
}